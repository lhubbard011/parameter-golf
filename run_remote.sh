...
# This avoids SSH stdout buffering entirely.
RUN_ID="$(echo "$DESC" | tr ' ' '_' | tr -cd 'a-zA-Z0-9_-')"
ssh $SOPTS ubuntu@$IP "
cd ~/parameter-golf &&
$ENV_VARS RUN_ID=$RUN_ID python3 train_gpt.py > run_${RUN_ID}.log 2>&1
...
if [[ -z "$VAL_BPB" ]]; then
    ...
    printf "Subject: pgolf\n\n[CRASH] $DESC" | msmtp "$SMS_TO" 2>/dev/null || true
    exit 1
fi
